...
class ContactFieldMapping(models.Model):
    _name = "contact.field.mapping"
    _description = "Contact Field Mapping"
    ...
    hubspot_fields = fields.Many2one('hubspot.contact.fields', domain=[('hubspot_compute', '!=', True), ('field_type', '=', 'calculation_equation'), ('hubspot_readonly', '!=', True)], string='Hubspot Field')
        ...
        for rec in self:
            if rec.odoo_fields:
                if rec.odoo_fields.ttype == 'char':
                    ...
                    return {'domain': {'hubspot_fields': [('hubspot_instance_id', '=', self.env.context.get('hubspot_instance_id')), '|', '|', '|', ('field_type', '=', 'checkbox'), ('field_type', '=', 'select'),
                                           ('field_type', '=', 'radio'), ('field_type', '=', 'booleancheckbox')]}}
