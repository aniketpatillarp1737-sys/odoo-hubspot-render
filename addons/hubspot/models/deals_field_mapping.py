...
class DealsFieldMapping(models.Model):
    _name = "deals.field.mapping"
    _description = "Deals Field Mapping"

    hubspot_instance_id = fields.Many2one('hubspot.instance')
    odoo_fields = fields.Many2one('ir.model.fields', domain=[('model_id.model', '=', 'crm.lead'), ('readonly', '!=', True)], string='Odoo Field')
    hubspot_fields = fields.Many2one('hubspot.deals.fields', domain=[('hubspot_compute', '!=', True), ('field_type', '=', 'calculation_equation'), ('hubspot_readonly', '!=', True)], string='Hubspot Field')
    ...
    def _onchange_hubspot_fields(self):
        ...
        if self.hubspot_fields:
            if self.hubspot_fields.field_type == 'text':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), '|', ('ttype', '=', 'text'), ('ttype', '=', 'char')]}}
            if self.hubspot_fields.field_type == 'phonenumber':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), ('ttype', '=', 'char')]}}
            if self.hubspot_fields.field_type == 'textarea':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), ('ttype', '=', 'text')]}}
            if self.hubspot_fields.field_type == 'checkbox' or self.hubspot_fields.field_type == 'select' or self.hubspot_fields.field_type == 'radio' or self.hubspot_fields.field_type == 'booleancheckbox':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), ('ttype', '=', 'selection')]}}
            if self.hubspot_fields.field_type == 'number':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), ('ttype', '=', 'integer')]}}
            if self.hubspot_fields.field_type == 'date':
                return {'domain': {'odoo_fields': [('model_id.model', '=', 'crm.lead'), ('ttype', '=', 'date')]}}
